...
// ============================================================
// GENERATED EXERCISE PARAMETERS
// ============================================================

export interface SustainedExhaleGeneratedParams {
...
interface SustainedExhaleScreenProps {
  /**
   * Current unlocked difficulty tier.
   */
  tier?: Tier;

  /**
   * Parameters generated by the Exercise Session Manager.
   *
   * When supplied, the screen uses these exact
   * parameters instead of generating another instance.
   */
  generatedParams?: SustainedExhaleGeneratedParams;
}
...
export default function SustainedExhaleScreen({
  ...
  /*
   * If the Exercise Session Manager provides parameters,
   * use them.
   *
   * Otherwise generate a standalone exercise instance.
   *
   * This fallback is useful while the rest of the session
   * manager is still being connected.
   */
  const generatedExerciseParams =
    useMemo(
      () =>
        generatedParams ??
        generateSustainedExhaleParams(tier),
      [generatedParams, tier]
    );

  const targetDuration =
    generatedExerciseParams.targetDurationSec;

  /*
   * Give the singer a few extra seconds beyond the target.
   *
   * Example:
   *
   * target = 17 sec
   * recording limit = 20 sec
   *
   * This allows the singer to slightly exceed the target
   * without keeping the microphone recording indefinitely.
   */
  ...
  const handleRecordingStop =
    useCallback(
      async (
        ...
        /*
         * Prevent duplicate processing if stopRecording()
        ...
        try {
           ...
           * IMPORTANT:
           *
           * The score uses the generated target duration.
           *
           * This means ADS can change the target while
           * the scoring system still uses the same scoring
           * method.
           */
          const score =
            scoreSustainedExhale(
          ...
          if (
            ...
            /*
             * Save exactly ONE exercise record after all
             * repetitions have been completed.
             *
             * The saved score is the overall exercise score,
             * not just the final repetition score.
             */
            try {
              ...
              /*
               * Progress saving should never prevent the
               * singer from seeing their results.
               */
              console.error(
            ...
            /*
             * Small delay so the processing screen is visible
        ...
        try {
          ...
          recordingTimerRef.current =
            setInterval(() => {
              ...
              /*
               * Stop at the generated target + small buffer.
               *
               * We do NOT use the original tier maximum here,
               * because ADS may have generated a different target.
               */
              if (
                seconds >=
                recordingLimit
  ...
  const finishRecording =
    useCallback(() => {
      ...
      /*
       * Let handleRecordingStop() perform the actual
       * processing.
       */
      finishingRef.current =
        false;
        ...
        >
          ...
          >
            ...
            <MetricCard
              icon="repeat-outline"
              label="Repetitions"
              value={`${repResults.length}/${tierParams.repetitions}`}
            />
...
function getFeedback(
  ...
  if (averageDuration < range[0]) {
    return 'Your exhale duration was below the target. Focus on taking a comfortable breath and releasing the air more gradually.';
  }
